...
public final class SkyBlockItemParser {
    ...
    public SkyBlockItemData parse(ItemStack stack, List<String> tooltipLines) {
        ...
        // 4. Collect NBT enchant IDs for line classifier to consume vanilla blocks
        Set<String> nbtEnchantIds = new HashSet<>();
        for (String key : enchNbt.keySet()) {
            nbtEnchantIds.add(key.toLowerCase());
        }

        // 5. Classify tooltip lines into typed sections (pass enchant IDs for dedup)
        TooltipLineClassifier.ParsedSections sections =
                classifier.classify(tooltipLines, nbtEnchantIds);

        // 6. Rarity (tooltip last line — no NBT equivalent)
        Rarity rarity = sections.rarity != null ? sections.rarity : Rarity.COMMON;

        // 7. Item type (NBT id primary, tooltip type tag fallback)
